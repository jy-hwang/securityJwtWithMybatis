...
public class TokenProvider {
	...
	public TokenProvider(String secret, long tokenValidityInSeconds) {
		...
		this.key = Keys.hmacShaKeyFor(keyByets);
	}
	...
	public String createToken(Authentication authentication) {
		...
		return Jwts.builder()
				...
				.claim(AUTHOIRIES_KEY, authorities)
	...
	/*
	 * 토큰을 받아서 클레임을 생성 클레임에서 권한 정보를 가져와서 시큐리티 UserDetails 객체를 만들고 Authentication 객체
	 * 반환
	 * 
	 * @param token
	 * 
	...
	public Authentication getAuthentication(String token) {
		...
		//@formatter:off
			Arrays.stream(claims.get(AUTHOIRIES_KEY).toString().split(","))
